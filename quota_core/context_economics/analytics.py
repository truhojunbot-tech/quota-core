...
# policy-relevant failures with provider outages, dispatcher bugs, and
# cancellations (the agent_crew#205/#206 AGY "subscriber fell behind"
# incident is the motivating real-world case -- 31 tester failures that were
# a transient streaming/backpressure error, not evidence any context was
# stale). These buckets partition every "not succeeded" record by
# `failure_category` so a context-age/compact-policy report can read
# `policy_relevant_failure_rate` instead of `raw_failure_rate` when judging
# whether context handling caused failures.
_PROVIDER_OR_RUNTIME_CATEGORIES: frozenset[str] = frozenset({"provider_or_transport", "runtime_or_dispatcher"})
_POLICY_RELEVANT_CATEGORIES: frozenset[str] = frozenset({"context_or_policy", "work_product_or_test"})
...
def stratified_failure_rates(records: Iterable[TaskEconomicsRecord]) -> dict[str, float | int | None]:
    ...
    failures -- not evidence about context/policy), `policy_relevant`
    (context/policy causes and work-product/test failures -- the signal
    ...
    bucket_counts: dict[str, int] = {"provider_or_runtime_operational": 0, "policy_relevant": 0, "cancelled": 0, "unknown": 0}
    for r in failed:
...
def context_age_vs_failure_rate(records: Iterable[TaskEconomicsRecord]) -> list[dict[str, float | int | None | str]]:
    ...
    for age, group in sorted(by_age.items()):
        ...
        if stratified["raw_failure_count"] and stratified["unknown_count"] == stratified["raw_failure_count"]:
            row["warning"] = UNKNOWN_FAILURE_CAUSE_WARNING
        rows.append(row)
    return rows
...
def compare_context_policies(records: Iterable[TaskEconomicsRecord]) -> dict[str, dict[str, float | int | None | str]]:
    ...
    (quota-core issue #60), plus a ``"warning"`` key when every failure
    observed under a policy has an unrecognized cause -- so a resume/compact/
    fresh comparison never gets read as context-policy evidence when the
    only thing actually known is an undifferentiated raw failure rate.
    """
    ...
    for policy, rows in by_policy.items():
        ...
        if stratified["raw_failure_count"] and stratified["unknown_count"] == stratified["raw_failure_count"]:
            entry["warning"] = UNKNOWN_FAILURE_CAUSE_WARNING
        result[policy] = entry
    return result
