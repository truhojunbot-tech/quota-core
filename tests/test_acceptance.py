...
def _row(index: int) -> dict[str, object]:
    ...
    return {"task_id": f"task-{index:03}", "created_at": 1000 + index, "runtime": "runtime-a" if index % 2 else "runtime-b", "risk_facts": {"safety_or_live_change": False, "broad_architecture_change": False, "bounded_routine_fix": routine, "human_gate_required": False}, "evidence_provenance": {"required_context_recalled": "observed_true", "recall_applicability": "observed_true", **{field: "producer_declared:explicit/high" for field in RISK}}, "policy_decision": decision}
...
def _passing_report() -> dict[str, object]:
    ...
    rows[0]["risk_facts"] = {field: None for field in RISK}
