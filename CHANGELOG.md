...
- Applying an operator risk declaration now uses `dataclasses.replace`, so it
  changes only declared risk facts and retains review-progress, context-growth,
  and orchestration-waste evidence that controls the review-round and session
  recommendations.
- A review task whose recorded context points to itself cannot supply its own
